...
using UnityEngine;
using System; // <- necesario para eventos opcionalmente
using UnityEngine.Events; // <- por si se usan UnityEvents
...
namespace bullet.fx.pack
{
    ...
    public sealed class Bullet : MonoBehaviour
    {
        ...
        public System.Action<Bullet> onReleaseToPool;

        private bullets.BulletPool bulletPool;
        ...
        public void Init(bullets.BulletPool pool, float lifetime)
        {
            bulletPool = pool;
        ...
        public void Fire()
        {
            ...
            if (BulletEffectType == BulletEffectType.Fire1)
                trailCoroutine = StartCoroutine(StartCreateBulletTrail());
        }
        ...
        private void CreateBulletTrail(Vector3 start, Vector3 end)
        {
            ...
            for (int i = 0; i < line.positionCount; i++)
            {
                ...
                line.SetPosition(i, Vector3.Lerp(start, end, t) + UnityEngine.Random.insideUnitSphere * 0.005f);
;
            }
        ...
        {
            Debug.Log($"[Bala] Colisionó con: {collision.collider.name}");

            // Evitar daño a quien disparó o a aliados
            if (owner != null)
            {
                if (collision.transform.root == owner.transform.root)
                {
                    Debug.Log($"[Bala] Ignorada: colisión con el propio dueño ({collision.gameObject.name})");
                    return;
                }

                if (collision.gameObject.CompareTag(owner.tag))
                {
                    Debug.Log($"[Bala] Ignorada: colisión con aliado ({collision.gameObject.name})");
                    return;
                }
            }
            ...
            {
                Debug.Log($"[Bala] Aplicando daño a {collision.collider.name} (Player)");
            ...
            else
            {
                ...
                if (enemyHealth != null)
                {
                    Debug.Log($"[Bala] Aplicando daño a {collision.collider.name} (Enemy)");
                    enemyHealth.TakeDamage(damageAmount);
                }
                else
                {
                    Debug.Log($"[Bala] {collision.collider.name} no tiene Health ni PlayerHealth");
                }
            }
        ...
        public void SetLifetime(float time) => maxLifetime = time;
        ...
        private void StartFadeAndReturn()
        {
            IsFlying = false;
